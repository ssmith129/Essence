type ORecord = Record<string, unknown>

function isObject(value: unknown): value is ORecord {
  ...
  for (const source of sources) {
    if (isObject(source)) {
      for (const key in source) {
        ...
        const targetValue = (target as ORecord)[key]

        if (isObject(sourceValue)) {
          if (!isObject(targetValue)) (target as ORecord)[key] = {}
          merge((target as ORecord)[key], sourceValue)
        } else (target as ORecord)[key] = sourceValue
      }
    }
  }
