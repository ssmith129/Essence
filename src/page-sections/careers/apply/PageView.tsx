...
const validationSchema = Yup.object({
  ...
  cv: Yup.mixed()
    .nullable()
    .test('required', 'CV is required!', (value) => (value ? true : false)),
  coverLetter: Yup.mixed()
    .nullable()
    .test('required', 'Cover Letter is required!', (value) => (value ? true : false)),
})
...
export default function CareerApplyPageView() {
  ...
  const handleDrop = useCallback(
    (field: 'cv' | 'coverLetter') => (files: File[], rejections: any[]) => {
      if (rejections.length > 0) {
        setError(field, { message: rejections[0].errors[0].message })
      } else {
