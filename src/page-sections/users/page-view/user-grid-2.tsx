...
export default function UserGrid2PageView() {
  ...
  const isUserSelected = useCallback((id: string) => selectedUser.id === id, [selectedUser])
        ...
        <Grid size={{ lg: 9, md: 8, xs: 12 }}>
          <Card sx={{ px: 3, height: '100%', borderTopRightRadius: 0, borderBottomRightRadius: 0 }}>
            <SearchArea
              ...
              gridRoute="/dashboard/users/user-grid-2"
              listRoute="/dashboard/users/user-list-2"
            />

            <Grid container spacing={3}>
              {paginate(page, userPerPage, filteredUsers).map((user) => (
                <Grid size={{ lg: 4, sm: 6, xs: 12 }} key={user.id}>
                  <UserGridCardTwo
                    user={user}
                    isSelected={isUserSelected(user.id)}
                    handleSelectUser={() => handleSelectUser(user.id)}
                  />
                </Grid>
              ))}
