...
export default function TodoColumn({ columnId, column, todos }: Props) {
  ...
  return (
    <TodoColumnRoot>
      {columnId === 'todo' ? (
        ...
        <Typography variant="body1" fontWeight={500} p={2}>
          {column.name}
        </Typography>
      )}
          ...
          {(provided) => (
            <DroppableWrapper ref={provided.innerRef} {...provided.droppableProps}>
              {column.todoIds.map((todoId, index) => (
                <TodoCard key={todoId} index={index} todo={todos[todoId]} />
              ))}
