...
type Filters = 'status' | 'searchValue'
...
export default function useProjects() {
  ...
  const handleChangeFilter = useCallback((key: Filters, value: string) => {
  ...
  const filteredProjects = useMemo(() => {
    ...
    return PROJECTS.filter((project) => {
      ...
      const searchMatch =
        searchValue === '' || project.name.toLowerCase().includes(searchValueLower)
