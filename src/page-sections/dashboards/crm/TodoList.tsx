...
export default function TodoList() {
  const [todos, setTodos] = useState([
    { id: 1, title: 'Design a poster for a company', complete: true },
    { id: 2, title: 'Analyze Data', complete: false },
    { id: 3, title: 'YouTube campaign', complete: false },
    { id: 4, title: 'Assign employee', complete: false },
  ])

  const handleCompleteTodo = useCallback(
    (id: number) => () => {
      setTodos((state) =>
        state.map((item) => (item.id === id ? { ...item, complete: !item.complete } : item))
      )
    },
    [setTodos]
  )
