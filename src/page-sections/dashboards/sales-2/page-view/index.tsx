...
// CUSTOM DATA
const CARD_LIST = [
  { trend: 'up', amount: currency(35800), percentage: 10.23, title: 'Revenue' },
  { trend: 'up', amount: currency(12900), percentage: 20.4, title: 'Repeat Purchase' },
  { trend: 'down', amount: currency(1000), percentage: 10.23, title: 'Average Order value' },
  { trend: 'down', amount: currency(143), percentage: 10.23, title: 'New Customers' },
]
      ...
      <Grid container spacing={3}>
        ...
        <Grid container spacing={3} size={{ lg: 6, xs: 12 }}>
          {CARD_LIST.map((item, index) => (
            <Grid key={index} size={{ lg: 6, md: 3, sm: 6, xs: 12 }}>
              <InfoCard
                trend={item.trend}
                title={item.title}
                amount={item.amount}
