...
export default function ProductGridPageView() {
  ...
  const handleChangeTab = useCallback((_: SyntheticEvent, newTab: string) => {
    setSelectTab(newTab)
    setPageIndex(1)
  }, [])
    ...
    <div className="pt-2 pb-4">
      <TabContext value={selectTab}>
        ...
        <SearchArea
          ...
          gridRoute="/dashboard/products/product-grid-view"
          listRoute="/dashboard/products/product-list-view"
        />

        <Grid container spacing={3}>
          {/* PRODUCT CARD LIST */}
          {filteredProducts.map((product) => (
            <Grid size={{ md: 3, sm: 6, xs: 12 }} key={product.id}>
