...
export default function LayoutSetting() {
  ...
  const handleLayoutChange = (layoutName: string) => {
    saveSettings({ ...settings, activeLayout: layoutName } as ThemeSettings)
    toggleDrawer()
  }
    ...
    <Fragment>
      ...
      <CustomDrawer open={open} anchor="right" variant="persistent">
        ...
        <FlexBox p={2} flexDirection="column">
          {demoLayouts.map(({ name, title, imgUrl }) => (
            <LayoutBox
              key={name}
              onClick={() => handleLayoutChange(name)}
              className={settings.activeLayout === name ? 'selected' : ''}
            >
...
const demoLayouts = [
  { name: 'layout1', title: 'Layout V1', imgUrl: '/static/layouts/layout-1.jpg' },
  { name: 'layout2', title: 'Layout V2', imgUrl: '/static/layouts/layout-2.jpg' },
]
