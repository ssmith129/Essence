...
export default function NotificationsPopover() {
  ...
  const handleTabChange = useCallback((_: SyntheticEvent, value: string) => setTabValue(value), [])
  ...
  const RENDER_CONTENT = () => {
    return (
      <TabContext value={tabValue}>
        ...
        {MESSAGES.length === 0 ? (
          <Typography variant="body2" fontWeight="500" textAlign="center" p={2}>
...
function ListItem({ image, name, message, type }: ListItemProps) {
  const isNew = type === 'new_message'

  return (
    <ListItemWrapper isNew={isNew}>
      <div className="thumbnail">
        <div className="dot" />
        <Avatar src={image} alt={name} sx={{ width: 35, height: 35 }} />
      </div>
